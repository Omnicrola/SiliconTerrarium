...
public class ExampleNeuralNetwork {
	...
	public Vector<Float> update(Vector<Float> inputs) {
		...
		for (int i = 0; i < this.hiddenLayersCount; i++) {
			...
			for (int j = 0; j < this.layers.get(i).neuronCount; j++) {
				float netInput = 0;
				final INeuralInput singleNeuron = this.layers.get(i).neurons.get(j);
				final int inputCount = singleNeuron.inputCount;
				for (int k = 0; k < inputCount; k++) {
					netInput += singleNeuron.weights[k] * inputs.get(weight++);
				}
				netInput += singleNeuron.weights[inputCount - 1] * D_BIAS;
	...
	private Float sigmoid(float netInput, double response) {
		final float P = 1.0f; // lower values look more like a step function
		final double output = 1 / (1 + Math.exp(-netInput / P));
		return (float) output;
	}
